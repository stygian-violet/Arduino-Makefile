...
LOCAL_C_SRCS    ?= $(call rwildcard,$(SRC_DIR)/,*.c)
LOCAL_CPP_SRCS  ?= $(call rwildcard,$(SRC_DIR)/,*.cpp)
LOCAL_CC_SRCS   ?= $(call rwildcard,$(SRC_DIR)/,*.cc)
LOCAL_PDE_SRCS  ?= $(call rwildcard,$(SRC_DIR)/,*.pde)
LOCAL_INO_SRCS  ?= $(call rwildcard,$(SRC_DIR)/,*.ino)
LOCAL_AS_SRCS   ?= $(call rwildcard,$(SRC_DIR)/,*.S)
...
ifeq ($(strip $(NO_CORE)),)
    ifndef MONITOR_BAUDRATE
        ifeq ($(words $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS)), 1)
            SPEED = $(shell grep -h -E 'Serial.begin *\([0-9]+\)' $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS) | sed -e 's/[^0-9]//g'| head -n1)
